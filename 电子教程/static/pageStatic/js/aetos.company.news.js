...
function addCompanyNews(_json){
	...
	var opt={
		html:'<div class="cnews"><a href="{url}" target="_blank"><span class="limg" style="background:url({titleImg}) no-repeat"></span><span class="rcont"><strong class="fonw">{title}<div class="feitime">{date}</div></strong><br />{content}</span></a></div>',
		div:'#news-content',
		addMore: '.news-add-more'
	}
	if(_json.message != 'success'){
		$(opt.addMore).removeClass('news-add-more').addClass('news-no-more');
	}
	var _node = opt.html,_html='';
	$.each(_json.data.annList,function(i,obj){
		var _htmlNode = '';
		if(obj.titleImg == '' || obj.titleImg == undefined){
			obj.titleImg = 'static/css/img/c-news.jpg'/*tpa=http://www.aetoscg.com/cn/static/pageStatic/js/static/css/img/c-news.jpg*/;
		}
		var valTitle = "";
		var urlTile = "";
		urlTile = obj.enTitle;
		urlTile = urlTile.replace(/[,]/g,'');
		urlTile = urlTile.replace(/[ ]/g,'-');
		urlTile=urlTile.toLocaleLowerCase();
		if(obj.title.length>35){
			valTitle = obj.title.substr(0,35) + '...';
		}else{
			valTitle = obj.title;
		}
		var objContent = removeHTMLTag(obj.content);
		if(objContent.length>200){
			valContent = objContent.substr(0,200) + '...';
		}else{
			valContent = objContent;
		}
		_html+=_node.replace('{titleImg}',obj.titleImg).replace('{url}','company-news-content.html?id='+obj.id+'&title='+urlTile).replace('{title}',valTitle).replace('{content}',valContent).replace('{date}',obj.createTimeFormat);
	});
	//把padding 图片去掉
